...
public class JwtTokenProvider {
    ...
    public TokenInfo generateToken(Authentication authentication, String userName, boolean autoLogin) {
        // 회원의 권한 가져오기
        ...
        String accessToken = Jwts.builder()
                ...
                .setExpiration(new Date(System.currentTimeMillis() + 6 * 3600 * 1000)) // 유효 기간 6시간
                .signWith(key, SignatureAlgorithm.HS256)
                .setHeaderParam("typ","JWT")
                .compact();
        
        // Refresh Token 생성
        String refreshToken = generateRefreshToken(autoLogin);
    ...
    // refresh 토큰 생성
    public String generateRefreshToken(boolean autoLogin) {
    	
        int expirationTime = autoLogin ? 365 * 24 * 3600 * 1000 : 24 * 3600 * 1000;
        
        // Refresh Token을 생성하고 서명
        String refreshToken = Jwts.builder()
                .claim("autoLogin", autoLogin)
        		.setId(UUID.randomUUID().toString()) // 고유 ID 설정
                .setExpiration(new Date(System.currentTimeMillis() + expirationTime)) // 유효 기간 30일 
                .signWith(key, SignatureAlgorithm.HS256)
                .compact();
	...
	/** 사용자 비밀번호 인증 확인을 위한 jwt 토큰 생성
	 * 
	 * @param userId 사용자 ID
     * @param purpose 목적 (예: "passwordChecked")
     * @return 생성된 JWT 토큰
	 */
	public String generatePasswordToken(String userId, String purpose) {
    ...
    * JWT 토큰에서 목적 추출
    * @param token JWT 토큰
    * @return purpose 목적 (예: "passwordChecked")
    */
   public String extractPurpose(String token) {
       return (String) Jwts.parserBuilder()
               ...
               .parseClaimsJws(token) 
               .getBody()
               .get("purpose"); // "purpose" 클레임 반환
   }
    
	/**
	 * Refresh 토큰에서 자동 로그인 여부 추출
	 * @param refreshToken
	 * @param key
	 * @return autoLogin
	 */
