...
public class CommunityRestController {
	...
	public ResponseEntity<CommunityVO> getBoardDetail(@PathVariable("com_no") Long com_no) throws Exception{
		if (com_no == null) { 
	...
	public ResponseEntity<String> writeBoard(@RequestBody CommunityVO com_vo) throws Exception{
		
		String errorMsg;
		if (com_vo == null) { 
			log.error("com_vo is null.");
			errorMsg = "com_vo is null.";
		...
		if (com_title == null) {
			...
			errorMsg = "com_title is null.";
		    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorMsg);
		}
		if (com_content == null) {
			log.error("com_content is null.");
			errorMsg = "com_content is null.";
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorMsg);
		}
		...
		try {
			...
			log.error("커뮤니티 글 작성 데이터 저장 중 오류 : " + responseBody);
	...
	public ResponseEntity<CommunityVO> getBoardById(@PathVariable Long com_no) {
		if (com_no == null) { 
			log.error("com_no is required");
			...
			try {
				...
				log.error("커뮤니티 수정 글 불러오던 중 오류 : " + e.getMessage());
	...
	public ResponseEntity<String> editBoard(@RequestBody CommunityVO com_vo) throws Exception{
		String errorMsg;
		if (com_vo == null) { 
			log.error("com_vo is null");
			errorMsg = "com_vo is null.";
	 ...
	 * 			성공 시 HttpStatus.OK를 반환하고 실패 시 HttpStatus.INTERNAL_SERVER_ERROR를 반환합니다.
	 **********************************/
	@RequestMapping(value="/board/{com_no}", method=RequestMethod.DELETE)
	public ResponseEntity<String> boardDelete(@PathVariable("com_no") Long com_no) throws Exception{
