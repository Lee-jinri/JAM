	...
	<script>
		$(function(){
			...
			$(document).on("click","button[data-btn='delBtn']",function(){
				...
				deleteBtn(comReply_no, com_no, loggedInUserId);
			});
		})
		
		function listAll(com_no, loggedInUserId) {
		    $(".reply").detach();
		    
		 	// 로그인 시 현재 user_id와 댓글 user_id 비교
		    if(loggedInUserId == null){
		    	url = "/comreplies/all/" + com_no;
		    }else url = "/comreplies/all/" + com_no + "?user_id=" + loggedInUserId;
		...
		/* 댓글 삭제 */
		function deleteBtn(comReply_no, com_no, loggedInUserId){
