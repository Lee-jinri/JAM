...
public class CommentController {
	...
	public List<CommentDto> commentList(@PathVariable("postId") Long postId, @AuthenticationPrincipal MemberDto user){
		String userId = (user != null) ? user.getUser_id() : null;
		
		List<CommentDto> comment = commentService.commentList(postId, userId);
		return comment;
	}
	...
	public ResponseEntity<Void> insertComment(@PathVariable("postId") Long postId, @RequestBody CommentDto c,  @AuthenticationPrincipal MemberDto user) {

		if(user == null) throw new UnauthorizedException("로그인이 필요한 서비스 입니다.");
	...
	public ResponseEntity<Void> updateComment(@PathVariable("commentId") Long commentId, @RequestBody CommentDto c, @AuthenticationPrincipal MemberDto user) {
	...
	public ResponseEntity<Void> deleteComment(@PathVariable("commentId") Long commentId, @AuthenticationPrincipal MemberDto user){
		
		if(user == null) throw new UnauthorizedException("로그인이 필요한 서비스 입니다.");
		
		commentService.deleteComment(commentId, user.getUser_id());		
